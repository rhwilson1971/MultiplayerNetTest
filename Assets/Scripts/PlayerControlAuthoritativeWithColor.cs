...
public class PlayerControlAuthoritativeWithColor : NetworkBehaviour, IPlayerActions
{
    [SerializeField]
    ...
    Vector2 moveDiection = Vector2.zero;
    ...
    {
        NetworkLog.LogInfoServer($"OK, De-spawning a client IsOwner={IsOwner} IsClient={IsClient} IsLocalPlayer={IsLocalPlayer}");
    }
    ...
    void IPlayerActions.OnMove(InputAction.CallbackContext context)
    {
        ...
        if (IsOwner && IsClient)
        {
            // moveDiection = context.ReadValue<Vector2>();
            // var velocity = new Vector2(moveDiection.x * moveSpeed, moveDiection.y * moveSpeed);
